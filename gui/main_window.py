...
class GameGUI2(QMainWindow):
    ...
    def initUI(self):
        ...
        # Left panel: Character info and inventory
        left_panel = QVBoxLayout()
        left_panel.addWidget(self.create_character_info_widget())
        left_panel.addWidget(self.create_inventory_widget())

        # Center panel: Game board and actions
        center_panel = QVBoxLayout()
        center_panel.addWidget(self.create_game_board_widget())
        center_panel.addWidget(self.create_action_buttons())

        # Right panel: Quest log and dialogue box
        right_panel = QVBoxLayout()
        right_panel.addWidget(self.create_quest_log_widget())
        right_panel.addWidget(self.create_dialogue_box())
    ...
    def create_character_info_widget(self):
        frame = QFrame()
        frame.setObjectName("character_info_frame")
        frame.setStyleSheet("background-color: #34495E; border: 2px solid #D4AF37; border-radius: 5px; padding: 10px;")
        layout = QVBoxLayout(frame)

        name_label = QLabel(self.player.name)
        name_label.setObjectName("name_label")
        name_label.setAlignment(Qt.AlignCenter)
        name_label.setFont(QFont("Trajan Pro", 18, QFont.Bold))
        name_label.setStyleSheet("color: #D4AF37;")
        ...
        for stat, label_text in stats:
            stat_layout = QHBoxLayout()
            stat_label = QLabel(f"{label_text}:")
            stat_label.setFont(QFont("Trajan Pro", 12))
            stat_value = QLabel(str(getattr(self.player, stat)))
            stat_value.setObjectName(f"{stat}_value")
            stat_value.setFont(QFont("Trajan Pro", 12, QFont.Bold))
            stat_value.setStyleSheet("color: #D4AF37;")
            stat_layout.addWidget(stat_label)
            stat_layout.addWidget(stat_value)
            layout.addLayout(stat_layout)

        health_label = QLabel(f"Health: {self.player.health}/100")
        health_label.setObjectName("health_label")
        health_label.setAlignment(Qt.AlignCenter)
        health_label.setFont(QFont("Trajan Pro", 14))
        health_label.setStyleSheet("color: #E74C3C;")
        layout.addWidget(health_label)

        gold_label = QLabel(f"Gold: {self.player.gold}")
        gold_label.setObjectName("gold_label")
        gold_label.setAlignment(Qt.AlignCenter)
        gold_label.setFont(QFont("Trajan Pro", 14))
        gold_label.setStyleSheet("color: #F1C40F;")
    ...
    def create_inventory_widget(self):
        ...
        title.setFont(QFont("Trajan Pro", 16, QFont.Bold))
    ...
    def create_game_board_widget(self):
        frame = QFrame()
        frame.setStyleSheet("background-color: #34495E; border: 2px solid #D4AF37; border-radius: 5px; padding: 10px;")
        layout = QVBoxLayout(frame)

        title = QLabel("Westeros Map")
        title.setAlignment(Qt.AlignCenter)
        title.setFont(QFont("Trajan Pro", 16, QFont.Bold))
        title.setStyleSheet("color: #D4AF37;")
        layout.addWidget(title)
        ...
        self.game_map = QGridLayout()
        self.update_game_map()
        layout.addLayout(self.game_map)
    ...
    def create_action_buttons(self):
        ...
        buttons = [
            ('Move', self.show_move_options),
            ('Interact', self.interact_with_npc),
            ('Use Item', self.use_item),
            ('View Quests', self.view_quests),
            ('Initiate Combat', self.initiate_combat)
        ]
    ...
    def create_quest_log_widget(self):
        frame = QFrame()
        frame.setStyleSheet("background-color: #34495E; border: 2px solid #D4AF37; border-radius: 5px; padding: 10px;")
        layout = QVBoxLayout(frame)
    ...
    def create_dialogue_box(self):
        ...
        self.dialogue_box = StyledTextEdit()
        layout.addWidget(self.dialogue_box)
        ...
        for y in range(self.game_board.size):
            for x in range(self.game_board.size):
                ...
                if (x, y) == self.player.position:
                    label.setStyleSheet(label.styleSheet() + "border: 2px solid #E74C3C;")
                self.game_map.addWidget(label, y, x)
    ...
    def update_character_info(self):
        # Find the character info frame
        for i in range(self.layout().count()):
            item = self.layout().itemAt(i)
            if isinstance(item, QVBoxLayout):
                for j in range(item.count()):
                    widget = item.itemAt(j).widget()
                    if isinstance(widget, QFrame) and widget.objectName() == "character_info_frame":
                        self.update_character_info_frame(widget)
                        return
        
        print("Character info frame not found")


    def update_character_info_frame(self, frame):
        # Update the labels in the character info frame
        for child in frame.findChildren(QLabel):
            if child.objectName() == "name_label":
                child.setText(self.player.name)
            elif child.objectName() == "health_label":
                child.setText(f"Health: {self.player.health}/100")
            elif child.objectName() == "gold_label":
                child.setText(f"Gold: {self.player.gold}")
            elif child.objectName().endswith("_value"):
                stat = child.objectName().split("_")[0]
                value = getattr(self.player, stat, "N/A")
